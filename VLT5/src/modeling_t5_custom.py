...
class VisualEmbedding(nn.Module):
    def __init__(self, config, obj_order_embedding):
        ...
        self.embeddingnet_0 = EmbeddingNet(config.d_model, config.d_model // 2)
        self.embeddingnet_1 = EmbeddingNet(config.d_model, config.d_model // 2)
        self.pool = maxpool
        self.activation = nn.ReLU()
        self.fc_c = nn.Linear(config.d_model, config.d_model)
    ...
    def forward(self, feats, pos, img_order_ids=None, obj_order_ids=None):
        ...
        vis_c = self.embeddingnet_0(vis_embedding) # in: d, out: d//2
        pooled = self.pool(vis_c, dim=1, keepdim=True).expand(vis_c.size())
        vis_c = torch.cat([vis_c, pooled], dim=2) # hdim = d

        vis_c = self.embeddingnet_1(vis_c)  # in: d, out: d//2
        pooled = self.pool(vis_c, dim=1, keepdim=True).expand(vis_c.size())
        vis_c = torch.cat([vis_c, pooled], dim=2)  # hdim = d

        vis_c = self.pool(vis_c, dim=1)
        vis_c = self.fc_c(self.activation(vis_c))

        return vis_embedding, vis_c
...
class JointEncoder(T5Stack):
    ...
    def forward(
        ...
        vis_embeds, vis_c = self.visual_embedding(
